...
class biSPOT:
    ...
    def initialize(self, verbose=False):
        ...
        # initial peaks
        self.peaks['up'] = self.init_data[self.init_data >
                                          self.init_threshold['up']]-self.init_threshold['up']
        self.peaks['down'] = -(self.init_data[self.init_data <
                               self.init_threshold['down']]-self.init_threshold['down'])
        ...
        if verbose:
            print('[done]')
            print('\t' + 'Parameters'.rjust(ltab) +
                  'Upper'.rjust(ltab) + 'Lower'.rjust(ltab))
            print('\t' + '-'*ltab*3)
            ...
            print(form % ('Extreme quantile',
                  self.extreme_quantile['up'], self.extreme_quantile['down']))
            print('\t' + '-'*ltab*3)
        return
    ...
    def _grimshaw(self, side, epsilon=1e-8, n_points=10):
        ...
        gamma_best = 0
        sigma_best = Ymean
        ll_best = biSPOT._log_likelihood(
            self.peaks[side], gamma_best, sigma_best)
    ...
    def plot(self, run_results, with_alarm=True):
        ...
        if 'lower_thresholds' in K:
            ...
            lth_fig, = plt.plot(
                x, thdown, color=deep_saffron, lw=2, ls='dashed')
            fig.append(lth_fig)
...
class bidSPOT:
    ...
    def initialize(self, verbose=False):
        ...
        # initial peaks
        self.peaks['up'] = T[T > self.init_threshold['up']] - \
    ...
    def _grimshaw(self, side, epsilon=1e-8, n_points=8):
        ...
        gamma_best = 0
        sigma_best = Ymean
        ll_best = bidSPOT._log_likelihood(
            self.peaks[side], gamma_best, sigma_best)
    ...
    def run(self, with_alarm=True, plot=True):
        ...
        for i in range(self.data.size):
            ...
            # upper thresholds record
            thup.append(self.extreme_quantile['up']+Mi)
            # lower thresholds record
